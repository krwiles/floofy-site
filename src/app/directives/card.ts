 ...
 * it's typed optional rather than forcing every hero usage to pass an arbitrary, unused tone value. `tone`,
 * `special`, `noBackground` and `glass` are not runtime-enforced as mutually exclusive -- misuse is a visible
 * CSS mistake, not a silent data bug (see docs/refactor/10-phase-3b-plan.md). Where more than one would apply,
 * `glass` wins over `noBackground`, which wins over the plain tone-painted surface.
 */
@Directive({
  selector: '[appCard]',
